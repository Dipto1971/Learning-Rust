...
fn longest_using_generic_lifetime <'a>(first: &'a str, second: &'a str) -> &'a str { //Lifespan of all the variables will be 'a
...
fn main() {
    let longest;
    let str1= String::from("Small passed in a function using lifetimes");
    let str2= String::from("Longer passsed in a function using lifetimes");

    longest = longest_using_generic_lifetime(&str1, &str2);
    println!("{}", longest);
}